...
class CocoDetection(FasterCocoDetection, DetDataset):
    ...
    def __init__(self, img_folder, ann_file, transforms, return_masks=False, remap_mscoco_category=False):
        ...
        self.img_folder, self.ann_file = img_folder, ann_file
        self.return_masks, self.remap_mscoco_category = return_masks, remap_mscoco_category
    ...
    def load_item(self, idx):
        ...
        path_v = os.path.join(self.img_folder, img_info['file_name'])
        path_ir = path_v.replace('img', 'imgr')
        if not os.path.exists(path_ir): path_ir = path_v.replace('.jpg', 'r.jpg')
        
        img_v, img_ir = Image.open(path_v).convert('RGB'), Image.open(path_ir).convert('RGB')
        ann_ids = self.coco.getAnnIds(imgIds=image_id)
        target_dict = {"image_id": image_id, "image_path": path_v, "annotations": self.coco.loadAnns(ann_ids)}
        ...
        if "boxes" in target_dict:
            target_dict["boxes"] = convert_to_tv_tensor(target_dict["boxes"], key="boxes", spatial_size=img_v.size[::-1])
...
class ConvertCocoPolysToMask(object):
    ...
    def __call__(self, image, target, **kwargs):
        from ...zoo.dfine.box_ops import poly2rbox # 局部导入
        ...
        for obj in anno:
            if "segmentation" in obj and len(obj["segmentation"]) > 0 and len(obj["segmentation"][0]) == 8:
                boxes.append(poly2rbox(torch.tensor(obj["segmentation"][0], dtype=torch.float32))) 
            else:
                b = obj["bbox"]
                boxes.append(torch.tensor([b[0]+b[2]/2, b[1]+b[3]/2, b[2], b[3], 0.0]))

        boxes = torch.stack(boxes).reshape(-1, 5) if boxes else torch.zeros((0, 5))
        ...
        return image, {
            "boxes": boxes[keep], "labels": labels[keep], "image_id": torch.tensor([target["image_id"]]),
            "image_path": target["image_path"], "area": torch.tensor([obj["area"] for obj in anno])[keep],
